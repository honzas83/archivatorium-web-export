...
```bash
VAULT_ROOT=/path/to/original/vault \
EXPORT_ROOT=/path/to/generated/export \
...
The server validates that every `sourcePath` stays inside `VAULT_ROOT`, only reads Markdown files, rewrites links to files outside the selected subset to `PUBLIC_ARCHIVE_ROOT`, and returns `vault-subset-YYYY-MM-DD.zip`.

## Export Settings

Enable `Shopping Basket` in the plugin settings for hosted exports and set the checkout endpoint if it is not `/api/checkout`.

The HTML export does not contain the original Markdown. The browser only sends source-path identifiers; the companion server must be able to read the original vault.
