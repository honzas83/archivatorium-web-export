...
export const RECORD_FORMAT_VERSION = "5";
...
function createDirectIndexWriter(database, recordStore) {
	...
	const insertSearch = database.prepare(`
		INSERT INTO search_documents(rowid, path, source_path, title, metadata, aliases, headers, tags, content)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`);
	...
	function writeRecord(filename, record) {
		...
		if (record.kind === "webpage" && record.search) {
			...
			insertSearch.run(
				documentId, data.exportPath, data.sourcePath, data.title ?? data.exportPath,
